...
#' @description
#' 
#' Adds a new state to a matrix. 
#' @usage
#' addStateMatrix(newMatrix, oldMatrix,stateName=paste("state", after+1, sep=" "),
#' after=dim(oldMatrix)[1], newCell="NULL")
#' @param 
#' \code{newMatrix}  an empty matrix with one more row and one more column than the old
#' matrix;
#' \code{oldMatrix}  a matrix to which we are adding a new state; 
#' \code{stateName} - the name of the state to be added; 
#' after
#' \code{newCell} - the value in the state to be added;
#' @return newMatrix
#' a matrix created by adding a new state with the value <newCell> 
#' to the old matrix
#' @examples
...
addState <- function(mypanel){
  ...
  if (no_adding){
    rp.messagebox("History as the parameter in hazard functions after state to be added.You have to redefine the model,
