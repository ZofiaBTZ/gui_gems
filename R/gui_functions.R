...
setStatesNum <- function(mypanel, num=0){
  ...
  mypanel$params <-  tryCatch(  #TODO: if does not work with proper mastrix, empty parameters matrix
    ...
    error = function(cond){
      ...
      rp.messagebox('Could not generate parameters matrix based on the hazard functions matrix
  ...
  mypanel$covariance <- tryCatch(
    ...
    error = function(cond){
      rp.messagebox('Could not generate parameters covariance matrix. Are the hazard functions defined?')
      return(NA)
    }
  )
...
saveModel <- function(mypanel){
  fileName <- tclvalue(tkgetSaveFile(defaultextension=".R", filetypes="{{R Data} {.RData}} {{All Files} {*.*}}"))
...
loadModel <- function(mypanel, fileName = NULL){
  ...
  if (is.null(fileName)){
    print(fileName)
    modelFile <- tk_choose.files(default = "", caption = "Select files",
...
multipleCohorts <- function(mypanel){ 
  ...
  mypanel$MCohorts <- rp.control(numStates=mypanel$numStates,
  ...
  rp.button(mypanel$MCohorts, addCohort, "Add cohort")
  rp.button(mypanel$MCohorts, editCohortsNames, "Edit cohorts names")
  rp.textentry(mypanel$MCohorts,costToView, action=ViewCost, initval = c(1), "View cost")
  rp.textentry(mypanel$MCohorts,subcohortToView, action=ViewSubcohort, initval = c(1), "View cohort")  
  rp.button(mypanel$MCohorts, listSubcohorts, "List cohorts")
  rp.button(mypanel$MCohorts , action=saveModel, "Save the model") 
  rp.combo(mypanel$MCohorts,functionToPlot, prompt = "", vals=c("transitionProbabilities", "cumulativeIncidence"))#,
  rp.textentry(mypanel$MCohorts,StatesToPlot, labels=c("states", "cohorts"),
             initval =c("\"c(1:4)\"", "\"c(1,2)\""),title = "Plot transition functions")
  rp.button(mypanel$MCohorts,plotFun, "Plot")          
  rp.button(mypanel$MCohorts, plotCostsMypanel, "Plot costs")
...
drawTable <- function(mypanel){
  ...
  for (i in seq(startRow,(startRow+nShow +8),1)){
    for (j in seq(startColumn,(startColumn+nShow+8),1)){
      ...
      rp.grid(mypanel, pos=list(row=i-1, column=j-1, width=60, height=20), name=grid_name)  
    }
  }
  maxnShow = 16 
  rp.button(mypanel,action=editTTT, "Edit ttt matrix",            pos = list (row = maxnShow + 2, column=5,columnspan = 2), name="editTTT")
  rp.button(mypanel,action=editStatesNames, "Edit states names",  pos = list (row = maxnShow + 3, column=5,columnspan = 2), name="editSN")
  
  rp.button(mypanel,action=editHf, "Edit hf matrix",              pos = list (row = maxnShow + 4, column=5,columnspan = 2), name="editHF")
  rp.button(mypanel,action=graphHf_mypanel, "Plot hf graph",      pos = list (row = maxnShow + 5, column=5,columnspan = 2), name="plotHFGraph")
  rp.button(mypanel,action=loadHf, "Load hazard functions",       pos = list (row = maxnShow + 5, column=1,columnspan = 2), name="lhf")
  rp.button(mypanel,action=loadBaselineFunction, "Load baseline", pos = list (row = maxnShow + 8, column=1,columnspan = 2), name="lbl")
  rp.button(mypanel,action=editBaselineFunction, "Edit baseline", pos = list (row = maxnShow + 9, column=1,columnspan = 2), name="lbl")
  
  rp.button(mypanel, action=saveModel, "Save the model",          pos = list (row = maxnShow + 6, column=1,columnspan = 2), name="saveModel")
  rp.button(mypanel,action=loadModel, "Load model",               pos=list(row = maxnShow + 8,    column=3,columnspan = 2), name="lmodel")
  rp.button(mypanel,action=updateMypanel, "Update panel",         pos=list(row = maxnShow + 9,    column=3,columnspan = 2), name="lmodel")
  
  rp.button(mypanel,action=multipleCohorts, "Multiple cohorts",   pos=list(row = maxnShow + 10,    column=3,columnspan = 2), name="MCohorts")
  num <- mypanel$numStates
  mypanel$num <- mypanel$numStates
  rp.textentry(mypanel, num, action=setStatesNum,                 pos = list(row = maxnShow + 3,  column =1,columnspan = 2),
               initval = num, "Number of states", name="lns")
  mypanel$cohortSize_input <- NULL
  cohortSize_input <- NULL
  a <- mypanel$cohortSize
  rp.textentry(mypanel, cohortSize_input, action=setCohortSize,   pos = list(row = maxnShow + 3,  column =3,columnspan = 2),
               initval= a, "Cohort Size", name="lns")
  rp.textentry(mypanel, max_time, action=setMaxTime,              pos = list(row = maxnShow + 5,  column =3,columnspan = 2),
               initval= max_time, "Time of simulation", name="to")
  View(mypanel$cohortSize)
  rp.button(mypanel, action = guiSimCoh ,                         pos = list(row = maxnShow + 6,  column =3,columnspan = 2), "Simulate Cohort", name = "sc")
  rp.textentry(mypanel, shiftXY, action=shiftWindowXY,            pos = list(row = maxnShow + 4,  column =1,columnspan = 4),labels =c("start rows","starts columns"),
               initval = c(1,1), name="shiftX")
  rp.textentry(mypanel,addingState, action = addState,            pos = list(row = maxnShow+3, column = 7,columnspan = 2, rowspan=2),
  ...
  for (i in seq(startRow,tmpr,1)){
    ...
    rp.text(mypanel, mypanel$statesNames[i],function(mypanel){mypanel}, pos=list(row=iGui, column=0))
  }
  
  for (j in seq(startColumn,tmpc,1)){
    jGui <- j - startColumn + 1
    rp.text(mypanel, mypanel$statesNames[j],function(mypanel){mypanel}, pos=list(row=0, column=jGui))
  }
  ...
  for (i in seq(startRow, tmpr, 1)){
    for (j in seq(startColumn, tmpc, 1)){
      ...
      if (i<j){
        ...
        rp.combo(mypanel, aa,  prompt=" ",
                 vals = myvals, action = open_hf, 
                 myInitVal, pos=list(row=iGui, column=jGui))
      }
      else {
        rp.text(mypanel, "  NULL",function(mypanel){mypanel}, pos=list(row=iGui, column=jGui))
      }
    }
  }
  mypanel
...
saveCost<-function(mypanel){
  fileName <- tclvalue(tkgetSaveFile(defaultextension=".RData",
...
loadCost<-function(mypanel){
 ...
 rp.messagebox("Cost file not chosen", title="No cost file" )
  return(mypanel)
}
...
loadHf <- function(mypanel){
  ...
  mypanel$hfDir <-tk_choose.dir(default = "./", caption = "Select directory with hazard functions")# file.choose()#(default = "./", caption = "Select a file with hazard functions")
  tmpwd <- getwd()
  setwd(mypanel$hfDir)
...
open_hf <- function(mypanel) {  
 ...
 if (hf_desc_list[[1]][1] =="hf"){
    ...
    fileName <- tclvalue(tkgetSaveFile(defaultextension=".R", filetypes="{{R scritpt {.R}} {{All Files} {*.*}}"))
    if (!fileName==""){ 
  ...
  if (mypanel$hfNames[st1,st2] %in% c("Weibull", "Exponential", "impossible"))
    ...
    functionName <- paste(mypanel$hfNames[st1, st2], ".R", sep="")
    fileName <- tclvalue(tkgetSaveFile(initialfile=functionName, defaultextension=".R",
                                       title="Save function..."))
    if (!nchar(fileName)) {
      tkmessageBox(message = "Function not saved!") 
    } else {
      saveFunctionFile(mypanel$hfNames[st1, st2], fileName)
...
showCohort <- function(mypanel){
  mypanel$cohPanel <- rp.control(cohort = mypanel$cohort, subcohorts=mypanel$subcohorts, numStates=mypanel$numStates,
                                 statesNames = mypanel$statesNames, hfNames = mypanel$hfNames, M = mypanel$M,
                                 cost = initCost(mypanel$numStates, mypanel$statesNames))  
  head(mypanel$cohort)
  rp.button(mypanel$cohPanel, editCohort, "Edit Cohort")
  rp.textentry(mypanel$cohPanel, cohortSize_input, updateCohort, "Update Cohort", initval = mypanel$cohort@size, width=80)
  rp.button(mypanel$cohPanel, listSubcohorts, "List subcohorts")
  rp.button(mypanel$cohPanel, listStates, "List states")
  rp.button(mypanel$cohPanel, saveModel, "Save the model" )
  rp.button(mypanel$cohPanel, plotPaths, "Plot paths" )
  rp.button(mypanel$cohPanel, plotPath_next, "Plot next paths")
  rp.textentry(mypanel$cohPanel, query, chooseSubcohort, 
               initval =substitute("(cohortBl[\"sex\"] ==1) & (cohortBl[\"age\"]>40)"),
               width = 80, "Add subcohort, eg. ")
  #  subcohorts_vals <- 
  rp.textentry(mypanel$cohPanel,subcohortToView, action=ViewSubcohort, initval = c(1), "View subcohort")  
  rp.combo(mypanel$cohPanel,functionToPlot, prompt = "", vals=c("transitionProbabilities", "cumulativeIncidence"))#,
  rp.textentry(mypanel$cohPanel,StatesToPlot, labels=c("states", "subcohorts"),
               initval =c("\"c(1:4)\"", "\"c(1,2)\""),title = "Plot transition functions")
  rp.button(mypanel$cohPanel,plotFun, "Plot")          
  rp.textentry(mypanel$cohPanel,statesToMerge, action = mergeStates_mypanel,labels=c("states goups"),
  ...
  rp.button(mypanel$cohPanel, action=editCostUnits, "Edit cost per unit",  name="editCostUnits")
  rp.button(mypanel$cohPanel, action=editQualy, "Edit QUALY per state",    name="editQUALY")
  rp.button(mypanel$cohPanel, action=editStateCost, "Edit cost per state", name="editStateCost")
  rp.button(mypanel$cohPanel, action=saveCost, "Save cost",                name="saveCost")
  rp.button(mypanel$cohPanel, action=loadCost, "Load cost",                name="loadCost")
  rp.button(mypanel$cohPanel, action=countCostMypanel, "Count cost",       name="Count Cost")
  mypanel
}
...
#' graphHf
#' @description
#' Plots the possible progression paths in the stochastic model
#' @usage
...
graphHf <- function(hfNames, statesNames=rownames(hfNames)){
  ...
  print("HfNames for a graph")
  print(adj)
  adjsq <- array(adj, dim=c(nrow(hfNames), nrow(hfNames)))
  print(adjsq)  
  ghf<-graph.adjacency(adjsq,  mode="directed")
  print(length(statesNames))
  V(ghf)$label <- statesNames  
  lay <- layout.reingold.tilford(ghf, circular=T)
...
statesPaths_cohort <- function(cohort){
  ...
  paths[!is.na(paths)]<-1
  agg_paths <- as.data.frame(count(paths))
  sortedPaths <- arrange(agg_paths, freq*(-1))
  return(sortedPaths)
}
...
color_path<-function(g,Vpath,w=5){
  E(g)$color<-"black"
  V(g)$color<-"rosybrown1"
  #V(g)$shape<-"rectangle"
  lg <- length(V(g))
  lp <- length(Vpath)
  V(g)[Vpath[lp]]$color<-"red"
  print("random print")
  print(Vpath)
  print(lp)
  E(g)$width <- 1
  if (lp > 1){
    for (f in seq(1:(lp-1))){
      ...
      print(low)
      print(up)
      E(g)[up%--%low]$color<-"red"
      E(g)[up%--%low]$width<-w
      V(g)[Vpath[f]]$color<-"red"
    }}
  lay <- layout.reingold.tilford(g, circular=T)
...
prepare_ggplot <- function(post, states_to_plot, mypanel ){
  ...
  check <- tryCatch(statesNames[states_to_plot], 
                    error=function(error){NULL})
  if (is.null(check)){
    rp.messagebox("Wrong States, choose different ones", title = "Wrong States")
...
#' merged_cohort (at) time.to.state is defined as time, when the patient arrives 
#' in the first state which was merged into the new state.
#' The function creates also a hfNames \code{matrix} for the merged states. They are used only to plot a graph of possible 
...
mergeStates <- function(cohort, statesGroups){ 
  ...
  if (any(hfNames[col(hfNames)<row(hfNames)]==TRUE))
    rp.messagebox( "Merging these states results in loops.
...
mergeStates_mypanel <- function(mypanel, statesGroups=NULL){
  ...
  subcohorts[["the main cohort"]] <- data.frame(as.data.frame(cohortGroup@baseline), 
                                                as.data.frame(tts))
  print(statesNames)
  mypanel$mergePanel <- rp.control(cohort = cohortGroup, subcohorts = subcohorts,statesNames=statesNames,
                                   numStates=statesNumberGroup, M=M, hfNames = hfNames)
  
  print(mypanel$mergePanel)
  rp.button(mypanel$mergePanel, editCohort, "Edit Cohort")
  rp.button(mypanel$mergePanel, listSubcohorts, "List subcohorts")
  rp.button(mypanel$mergePanel, listStates, "List states")
  rp.button(mypanel$mergePanel, plotPaths, "Plot paths")
  rp.button(mypanel$mergePanel, plotPath_next, "Plot next paths")
  rp.textentry(mypanel$mergePanel, query, chooseSubcohort, 
               initval =substitute("(cohortBl[\"sex\"] ==1) & (cohortBl[\"age\"]>40)"),
               width = 80, "Add subcohort, eg. ")
  rp.textentry(mypanel$mergePanel,subcohortToView, action=ViewSubcohort, initval = c(1), "View subcohort")  
  rp.combo(mypanel$mergePanel,functionToPlot, prompt = "", 
           vals=c("transitionProbabilities", "cumulativeIncidence"))#,
  rp.textentry(mypanel$mergePanel,StatesToPlot, labels=c("states", "subcohorts"),
               initval =c("\"c(1:4)\"", "\"c(1,2)\""),title = "Plot transition functions")
  rp.button(mypanel$mergePanel,plotFun, "Plot")          
