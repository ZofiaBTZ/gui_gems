...
# @name guiSimCoh
# @description
# The function to simulate cohort from the level of rp.panel.
...
guiSimCoh<- function(mypanel){
  ...
  if (updateM == TRUE){
    ...
    rp.messagebox(msg, title = "Undefined functions") 
    return(mypanel)
  }
  ...
  if (!ok){
    ...
    rp.messagebox(msg, title = "wrong parameters functions") 
    return(mypanel)
  } 
...
write.table(dframe, "dframe.dat")
 if (k < 7){
    print( ggplot(data=dframe, aes(x= time, y= probability, 
                          ymin=as.numeric(lower), ymax = as.numeric(upper),  
                          group=subcohort, colour=subcohort, fill=subcohort)) 
           + facet_wrap(~state, nrow=floor(sqrt(k)))+ geom_line()+geom_ribbon(alpha=.05) 
           + ggtitle(myfunction))
    ...
    for (f in seq(1:k)){
      ...
      print( ggplot(as.data.frame(dframe[dframe["state"]==state_name,]), aes(x= time, y= probability,
                                                                     ymin=as.numeric(lower), ymax = as.numeric(upper),  
                                                                     group=subcohort, colour=subcohort, fill=subcohort))+ 
               geom_line()+geom_ribbon(alpha=.05)+ ggtitle(state_title))
    }}
